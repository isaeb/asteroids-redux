...
def draw_wrapped_text(surface, text, font, color, rect):
    ...
    for paragraph in paragraphs:
        ...
        for line in lines:
            ...
            line_surf = renderText(line, font, startColor, endColor, SELECTED_SHADOW, (3, 3), (0, 0, 0), 1)
...
class ClassicUpgrades:
    ...
    def drawOptions(self, game):
        ...
        for i, text in enumerate(['Movement', 'Weapons', 'Special', 'Auxiliary']):
            ...
            textSurface = renderText(text, self.optionFont, startColor, endColor, shadowColor, (3, 3), (0, 0, 0), 1)
            ...
            startColor = UNSELECTED_TEXT_START
            endColor = UNSELECTED_TEXT_END
            textSurface = renderText(f'LVL{levels[i]}', self.optionFont, startColor, endColor, UNSELECTED_SHADOW, (3, 3), (0, 0, 0), 1)
            yy = y + thickness + i * (self.optionSize + thickness * 2)
    ...
    def updateControl(self, game):
        ...
        # Read player input
        if True in [keys[key] for key in game['controls']['keyFire']] and 'fire' not in pressedKeys:
            ...
            if self.state == 0:
                if self.selected < 4:
                    if False:#self.stars <= 0:
                        ...
                        return
                    if game[['movement', 'weapons', 'special', 'auxiliary'][self.selected]] < 9:
                        self.state = 1
                        self.confirm = ConfirmMenu(game, ['movement', 'weapons', 'special', 'auxiliary'][self.selected])
                elif self.selected == 4:
                    return True
        elif not True in [keys[key] for key in game['controls']['keyFire']] and 'fire' in pressedKeys:
            pressedKeys.remove('fire')

        if True in [keys[key] for key in game['controls']['keyUp']] and 'up' not in pressedKeys:
            pressedKeys.append('up')
            self.selected = max(self.selected - 1, 0)
        elif not True in [keys[key] for key in game['controls']['keyUp']] and 'up' in pressedKeys:
            pressedKeys.remove('up')
        
        if True in [keys[key] for key in game['controls']['keyDown']] and 'down' not in pressedKeys:
...
class ConfirmMenu:
    def __init__(self, game, category):
        ...
        for desc in self.desc:
            text = desc['desc']
            testSurf = self.descFont.render(text, True, pygame.Color(0, 0, 0))
            while testSurf.get_width() > game['screenHeight'] * 0.15 * (len(self.desc) + 1) * 5:
                self.descSize -= 1
                self.descFont = pygame.font.Font('fonts/Signwood.ttf', self.descSize)
                testSurf = self.descFont.render(text, True, pygame.Color(0, 0, 0))
